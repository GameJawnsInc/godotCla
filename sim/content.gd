...
const LOADOUTS := {
	"tender": {"name": "Tender", "desc": "lance, seed bomb, dash · the starter kit",
		"kit": ["solar_lance", "seed_bomb", "mycelium_dash"],
		"protect": ["mycelium_dash", "seed_bomb"], "requires": {}},
	"tidewarden": {"name": "Tidewarden", "desc": "water jet replaces the lance · shoves",
		"kit": ["water_jet", "seed_bomb", "mycelium_dash"],
		"protect": ["mycelium_dash", "seed_bomb"], "requires": {}},
	"flarekeeper": {"name": "Flarekeeper", "desc": "sun flare replaces the lance · lights oil",
		"kit": ["sun_flare", "seed_bomb", "mycelium_dash"],
		"protect": ["mycelium_dash", "seed_bomb"], "requires": {}},
	"spiker": {"name": "Spiker", "desc": "grow spike replaces the lance · pays off growth",
		"kit": ["grow_spike", "seed_bomb", "mycelium_dash"],
		"protect": ["mycelium_dash", "seed_bomb"], "requires": {}},
	"lasher": {"name": "Lasher", "desc": "vine whip replaces the lance · drags",
		"kit": ["vine_whip", "seed_bomb", "mycelium_dash"],
		"protect": ["mycelium_dash", "seed_bomb"], "requires": {}},
	"skyrunner": {"name": "Skyrunner", "desc": "gust and updraft replace the lance and dash",
...
const ITEMS := {
	"sun_capsule": {"name": "Sun Capsule", "desc": "+3 charge"},
	"balm_fruit": {"name": "Balm Fruit", "desc": "Heal 4 HP"},
	"spore_vial": {"name": "Spore Vial", "desc": "Stun enemies within 2"},
	"clearair_pod": {"name": "Clear-Air Pod", "desc": "-5 smog"},
	"iron_seed": {"name": "Iron Seed", "desc": "+3 shield"},
	# Upcycled forms: two consumables pressed into one at a shrine.
	"sun_capsule+": {"name": "Solar Battery", "desc": "+6 charge"},
	"balm_fruit+": {"name": "Orchard Feast", "desc": "Heal to full"},
	"spore_vial+": {"name": "Spore Storm", "desc": "Stun enemies within 4 for 2 turns"},
	"clearair_pod+": {"name": "Storm Seed", "desc": "-12 smog"},
	"iron_seed+": {"name": "Ironheart", "desc": "+3 shield, +1 max HP"},
}
...
const GRAFTS := {
	...
	"bloom_surge": {"name": "Bloom Surge", "desc": "+1 bloom per cleanse", "tags": ["growth", "economy"], "price": 3, "stat": {"cleanse_bloom": 1}},
	...
	"ember_sap": {
		"name": "Ember Sap", "desc": "1 dmg to whoever stands where fire starts · 3/turn",
		"tags": ["fire"], "price": 5,
		"hooks": [{"on": "ignite", "effects": [{"op": "damage_at", "dmg": 1}], "cap_per_turn": 3}],
	},
	"undertow": {
		"name": "Undertow", "desc": "staggered enemies are rooted a turn",
		"tags": ["water", "displace", "control"], "price": 4,
		"hooks": [{"on": "staggered", "effects": [{"op": "status_at", "status": "root", "turns": 1}]}],
	},
	"compost": {
		"name": "Compost", "desc": "kills leave growth where they fall",
		"tags": ["growth"], "price": 6,
		"hooks": [{"on": "kill", "effects": [{"op": "terrain_at", "kind": "growth"}]}],
	},
	"oil_tithe": {
		"name": "Oil Tithe", "desc": "first cast at oil each turn costs 1 less, min 1",
...
const RESONANCES := {
	"cinder_grip": {
		"name": "Cinder Grip", "desc": "roots an enemy a turn when fire starts under it · 3/turn",
		"tag": "fire", "need": 3,
...
const ABILITY_DESC := {
	"solar_lance": "Beam 3 tiles: 2 dmg, ignites oil",
	"solar_lance+noon": "Beam 4 tiles: 3 dmg, 4 under clear skies, ignites oil",
	"solar_lance+pierce": "Beam 4 tiles: 2 dmg to every enemy on the line, ignites oil behind them",
	"seed_bomb": "Plant healing growth within 3",
	"seed_bomb+tangle": "Plant healing growth within 3, rooting enemies on it 1 turn · Growth: +1 radius",
	"seed_bomb+reclaim": "Plant healing growth within 3 · corruption in the patch becomes growth",
	"vine_whip": "Pull an enemy 2 toward you, 2 dmg · interrupts",
	"vine_whip+lash": "Pull an enemy 3 toward you, 3 dmg · stun 1 turn if dragged through fire",
	"vine_whip+rake": "Pull every enemy on a 3-tile line 2 toward you, 2 dmg · interrupts",
	"water_jet": "Shove enemies 2, impact 2 dmg, interrupts · Growth: +1 push, +1 dmg",
	"water_jet+pin": "Shove enemies 3, impact 3 dmg and root 1 turn · Growth: +1 push, +1 dmg",
	"water_jet+sluice": "Wash oil, fire, ash off 5 tiles, shove 2, impact 2 dmg · Growth: +1 push, +1 dmg",
	"mycelium_dash": "Teleport to a growth tile within 4",
	"mycelium_dash+trail": "Teleport to a growth tile within 7, leaving growth behind",
	"mycelium_dash+scatter": "Teleport to a growth tile within 4, 1 dmg to everything beside you",
	"root_wall": "Wall of roots blocks enemies for 4 turns",
	"root_wall+bulwark": "Wall of roots blocks enemies for 6 turns",
	"root_wall+cage": "Roots fill every open tile beside an enemy within 3 for 4 turns",
	"pollen_burst": "Stun everything within 2 for 1 turn",
	"pollen_burst+torpor": "Stun everything within 2 for 2 turns",
	"pollen_burst+drift": "Stun everything within 2 of a tile up to 3 away for 1 turn",
	"sun_flare": "1 dmg within 2, +1 in fire, ignites oil · Growth: -1 cost, +1 radius",
	"sun_flare+corona": "2 dmg within 2, +1 in fire, ignites oil · Growth: -1 cost, +1 radius",
	"sun_flare+smoulder": "1 dmg within 2, +1 in fire, lit oil burns 4 turns · Growth: -1 cost, +1 radius",
	"thorn_shield": "2 shield · absorbs dmg before HP",
	"thorn_shield+plate": "3 shield · absorbs dmg before HP",
	"thorn_shield+chaff": "2 shield and 2 turns of smoke · screens you from 2+ tiles away",
	"overgrowth": "Turn corruption around a tile into growth",
	"overgrowth+sprawl": "Turn corruption within 2 of a tile up to 3 away into growth",
	"overgrowth+palisade": "Turn corruption around a tile into a wall of roots for 3 turns",
	"sap_snare": "Root an enemy within 3 for 2 turns",
	"sap_snare+tether": "Root an enemy within 4 for 3 turns",
	"sap_snare+blight": "1 dmg a turn for 3 turns to an enemy within 3 · doses stack",
	"grow_spike": "3-4 dmg to an enemy near growth within 3 · Growth: +1 dmg",
	"grow_spike+impale": "3-5 dmg to an enemy near growth within 4, +1 per growth by it · Growth: +1 dmg",
	"grow_spike+throng": "3-5 dmg to an enemy near growth within 3, +1 per enemy by it · Growth: +1 dmg",
	"spore_cloud": "Spore all within 2: 1 dmg a turn for 3 turns",
	"fungal_ring": "Growth on every tile around you",
	"burrow": "Tunnel to any open tile within 3",
	"tide": "Shove everything 2 away, impact 1 dmg",
	"steam_vent": "Smoke on a tile for 3 turns · blocks beams",
	"geyser": "1 dmg beside you, shove everything 2",
	"gust": "Shove everything on a 3-tile line back 3, clears smoke",
	"updraft": "Dash up to 3 in a straight line",
	"clear_air": "Clear smoke within 3, shove enemies back 1",
	"spore_cloud+": "Spore all within 3: 1 dmg a turn for 3 turns",
	"fungal_ring+": "Growth on every tile around you · roots enemies on it 1 turn",
	"burrow+": "Tunnel to any open tile within 4",
	"tide+": "Shove everything 3 away, impact 2 dmg and root 1 turn",
	"steam_vent+": "Smoke on a tile for 5 turns · blocks beams",
	"geyser+": "2 dmg beside you, shove everything 2, +1 in fire",
	"gust+": "Shove everything on a 4-tile line back 4, clears smoke",
	"updraft+": "Dash up to 4 in a straight line",
	"clear_air+": "Clear smoke within 4, shove enemies back 2",
	"bramble_coat": "Attackers take 2 dmg for 4 turns",
	"bramble_coat+briar": "Attackers take 3 dmg for 5 turns",
	"bramble_coat+bristle": "Attackers take 5 dmg for 2 turns",
	"anchor_roots": "Immune to drags for 4 turns",
	"anchor_roots+bedrock": "Immune to drags for 7 turns",
	"anchor_roots+heave": "Immune to drags for 4 turns · shove everything adjacent back 1",
	"moss_filter": "Restore a stage of regen lost to smog",
	"moss_filter+sieve": "Restore a stage of regen lost to smog",
	"moss_filter+prism": "Restore a stage of regen lost to smog · 1 dmg within 2 if the sky is then clear",
}
...
const MUTATORS := {
	...
	"no_lance": {"name": "Lance Embargo", "desc": "no Solar Lance in the kit, drafts or shrine",
		"config": {"pool_ban": ["solar_lance"], "kit_ban": true}},
	"wide_draft": {"name": "Wide Draft", "desc": "four offers per draft", "config": {"draft_offers": 4}},
	"upgrades_only": {"name": "Upgrades Only", "desc": "drafts offer only upgrades of what you hold",
...
const MILESTONES := [
	...
	{"id": "no_lance", "kind": "mutator", "requires": {"wins_without": ["solar_lance"]}, "desc": "Win without Solar Lance"},
